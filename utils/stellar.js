...
exports.stellar = (function() {
  ...
  createEscrowAccount = (secretKey, shaSignerKey, callback) => {
    ...
    (async () => {
      try{
        ...
        let encodedSignerKey = new Buffer(base32.encode(StellarSdk.StrKey.encodeSha256Hash(shaSignerKey)));
  ...
  acceptPayment = (shaSignerKey, escrowAccountKey, myAccountId) => {
    let encodedSignerKey = base32.encode(StellarSdk.StrKey.encodeSha256Hash(shaSignerKey))
    StellarSdk.Network.useTestNetwork();
    (async () => {
      ...
      try{
        ...
        transaction.signHashX(encodedSignerKey);
